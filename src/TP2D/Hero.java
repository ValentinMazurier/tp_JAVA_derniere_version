...
public final class Hero extends DynamicThings{
    ...
    private long invincibilityDuration = 3000; // Durée de l'invincibilité en millisecondes
    ...
    public void moveIfPossible(double dx, double dy, Dungeon dungeon){
        ...
        for (Things things : dungeon.getRenderList()){
            ...
            else if (things instanceof Enemy) {
                if(((Enemy) things).getHitBox().intersect(this.getHitBox())){
                    movePossible=false;
                    String s = "Be ready to die!";
                    break;
                }
            }
        }
    ...
    public void draw(Graphics g) {
        ...
        if (invincible) {
            g.setColor(Color.GREEN);
            g.drawString("Je suis touché !!", (int) x, (int) y - barHeight - 10);
        }
